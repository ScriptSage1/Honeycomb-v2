...
pi-ai 事件变成 harness 的推理、文本、工具调用、用量与 finish 分片。适配器把解析后的工具参数以原始 JSON 字符串传给 harness。路由设置 `promoteTextToolCalls: true` 时，会等提供方结束再发出该回复。当回复没有提供方工具调用，且恰好包含一个 `{ name, arguments }` 对象、`name` 是本次请求中的工具、`arguments` 符合该工具的 schema 时，适配器从文本中移除该对象，并把它作为工具调用发出，finish 原因为 `tool-calls`。缺少 `name` 时，字符串 `function` 字段提供工具名。这一轮省略提供方回放状态。零个匹配、多个匹配、未知名称，或参数超出 schema，都保持原回复不变。
