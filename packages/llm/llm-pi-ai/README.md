...
For a custom Gemini route using `api: openai-completions` and Google's
`https://generativelanguage.googleapis.com/v1beta/openai/` endpoint, set
`compat: { supportsStore: false }`. Google rejects the OpenAI `store` field,
including `store: false`. Empty HTTP 400/413 responses from non-Cerebras routes
are classified as invalid requests, not evidence of context overflow.
Singleton JSON error arrays from OpenAI-compatible endpoints are unwrapped
before the SDK consumes them, preserving the provider's explanation. This lets
exhausted Gemini quota reach the harness as `QUOTA` instead of an opaque,
retryable HTTP 429; temporary model unavailability remains `SERVER`.
...
| [`src/text-tool-call.ts`](src/text-tool-call.ts) | Optional promotion of one text-shaped `{ name, arguments }` object into a tool call |
| [`src/replay.ts`](src/replay.ts) | Versioned `ReplayEnvelope` storage and validation |
| [`src/google-thought-signature.ts`](src/google-thought-signature.ts) | Gemini 3 function-call thought signature fill before send |
...
pi-ai events become harness reasoning, text, tool-call, usage, and finish chunks. The adapter passes parsed tool arguments to the harness as raw JSON strings. A route with `promoteTextToolCalls: true` holds that reply until the provider finishes. When the reply has no provider tool call and contains exactly one `{ name, arguments }` object, `name` is a tool on the request, and `arguments` match that tool's schema, the adapter removes the object from the text and emits it as a tool call with finish reason `tool-calls`. A string `function` field supplies the tool name when `name` is absent. The provider replay state is omitted for that turn. Zero matches, several matches, an unknown name, or arguments outside the schema leave the reply unchanged.
...
- **`promoteTextToolCalls` holds the reply until the provider finishes** — token deltas are not forwarded while the route decides whether the text is one tool call. A schema the checker cannot read (`oneOf`, `anyOf`, `allOf`, `$ref`, `not`, or `patternProperties`) also leaves the reply as text.
- **A filled Gemini thought signature is not the model's original signature** — the skip value lets the follow-up request succeed when the stored call has none. Gemini then continues without that call's original reasoning signature.
