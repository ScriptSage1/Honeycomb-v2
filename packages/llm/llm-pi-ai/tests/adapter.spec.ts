...
describe('PiAiAdapter provider routing', () => {
  it.each([
    [429, 'You exceeded your current quota. Free daily request limit: 20.', 'QUOTA'],
    [503, 'This model is currently experiencing high demand.', 'SERVER'],
    [400, 'Invalid JSON payload received. Unknown name "store".', 'INVALID_REQUEST'],
  ])('preserves Google error envelopes for HTTP %s', async (status, message, code) => {
    const server = await mockServer([{ status, body: JSON.stringify([{ error: { code: status, message } }]) }])
    const ctx = await harness(server.url)
    const result = await assemble(ctx, { model: 'deepseek-v4-flash', messages: [] })
    expect(result.finish).toMatchObject({ kind: 'error', failure: { code, message: expect.stringContaining(JSON.stringify(message).slice(1, -1)) } })
