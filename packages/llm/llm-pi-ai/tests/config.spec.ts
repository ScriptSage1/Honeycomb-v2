...
describe('reasoning schema boundary', () => {
  ...
  it('accepts text-tool-call promotion as an optional route switch', () => {
    const enabled = routeWith({ promoteTextToolCalls: true })() as {
      providers: Record<string, { promoteTextToolCalls?: boolean }>
    }
    expect(enabled.providers['acme-gateway']?.promoteTextToolCalls).toBe(true)
    const absent = routeWith({})() as { providers: Record<string, { promoteTextToolCalls?: boolean }> }
    expect(absent.providers['acme-gateway']?.promoteTextToolCalls).toBeUndefined()
  })
