...
describe('mapStopReason / mapUsage', () => {
  it.each(['400 status code (no body)', '413 status code (no body)'])('does not infer Gemini context overflow from %s', (errorMessage) => {
    expect(mapStopReason(assistant({ provider: 'gemini', stopReason: 'error', errorMessage })))
      .toMatchObject({ kind: 'error', failure: { code: 'INVALID_REQUEST' } })
    expect(mapStopReason(assistant({ provider: 'cerebras', stopReason: 'error', errorMessage })))
