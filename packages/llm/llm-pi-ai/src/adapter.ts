...
import { fetchWithErrorEnvelope } from './error-response.ts'
import {
  attributionHeaders,
  contentHasImage,
  LlmAdapter,
...
export class PiAiAdapter extends LlmAdapter {
  ...
  private async * streamWithSnapshot(
    ...
    try {
      ...
      const translated = toStreamChunks(events, model.contextWindow, options.signal, model.id)
      const delivered = profile.promoteTextToolCalls === true
        ? promoteTextToolCallStream(translated, options.tools)
        : translated
      const iterator = delivered[Symbol.asyncIterator]()
      let exhausted = false
      try {
        while (true) {
