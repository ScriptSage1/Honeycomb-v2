...
export function mapStopReason(message: AssistantMessage, contextWindow?: number): FinishReason {
  // pi-ai's empty HTTP 400/413 heuristic is specific to Cerebras. Other
  // providers (including Gemini) also use these statuses for invalid fields.
  const emptyHttpError = /^4(?:00|13)\s*(?:status code)?\s*\(no body\)\s*$/i.test(message.errorMessage ?? '')
  const piAiOverflow = (!emptyHttpError || message.provider === 'cerebras')
