...
export interface PiAiProviderProfile {
  ...
  /**
   * Promote one text-shaped `{ name, arguments }` object into a tool call when
   * the provider finished with no tool call, `name` is one of the request
   * tools, and `arguments` match that tool's schema. Zero or several such
   * objects leave the text unchanged. The route emits the reply only after
   * the provider finishes.
   */
  promoteTextToolCalls?: boolean
}
...
const profile = z.object({
  ...
  promoteTextToolCalls: z.boolean(),
})
