...
describe('Hero chrome', () => {
  it('renders localized welcome actions while preserving the brand slot', () => {
    const renderSlot = vi.fn<HeroShellProps['renderSlot']>(() => null)
    const view = render(<HeroShell t={makeTranslate(en, commonEn)} renderSlot={renderSlot} />)
    expect(view.getByText(en['hero.headline'])).toBeTruthy()
    expect(view.getByText(en['hero.kicker'])).toBeTruthy()
  ...
  it('seeds a localized starter draft without submitting it', () => {
    const onStarter = vi.fn()
    const renderSlot = vi.fn<HeroShellProps['renderSlot']>(() => null)
    const view = render(<HeroShell t={makeTranslate(en, commonEn)} renderSlot={renderSlot} onStarter={onStarter} />)
    fireEvent.click(view.getByRole('button', { name: en['hero.action.review'] }))
    expect(onStarter).toHaveBeenCalledWith(en['hero.prompt.review'])
  })
})
...
describe('ConversationRoot resident composer', () => {
  ...
  it('hero phase: keeps sidebar controls accessible while hiding conversation chrome', () => {
    ...
    expect(b.view.getByText(zh['hero.kicker'])).toBeTruthy()
