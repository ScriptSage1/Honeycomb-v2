...
The final `honeycomb.css` sheet maps the Honeycomb palette, local font stacks, surfaces, elevation and motion onto existing semantic tokens. Dark is the new-install default; persisted light/system preferences remain supported. Component geometry stays in CSS Modules.
