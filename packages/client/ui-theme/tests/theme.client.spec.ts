...
function persistedSystem() {
  const host = stubSettingsScope<ThemeSettings>()
  const bootSize = Number.parseInt(document.body.style.getPropertyValue('--dsh-content-font-size'), 10)
  const fontSize = bootSize >= 12 && bootSize <= 17 ? bootSize : 14
  host.publish({ status: 'ready', value: { preference: 'system', fontSize }, revision: 1, writable: true })
  return host
}

const make = (host = persistedSystem()): {
  ctx: Context
  theme: ThemeRuntime
...
describe('ThemeRuntime', () => {
  it('uses dark for a new installation without a stored preference', () => {
    const { theme } = make(stubSettingsScope<ThemeSettings>())
    expect(theme.getTheme().preference).toBe('dark')
    expect(theme.getTheme().active.colorScheme).toBe('dark')
  })
  it('preserves a stored system preference resolved against prefers-color-scheme', () => {
    const { theme } = make()
